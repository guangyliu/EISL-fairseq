cuda=1
for i in 20 # 0 10 #20 # 25 
do
    base_noise="rsbs"
    preix=$base_noise
    sym="cetf_wmt_$i"test
    loss="cross_entropy" # cross_entropy
    model="bart_base" # "bart_base_iter"
    tf_ratio=1.0 #'1.0'
    noise="$preix$i"
    data="wmt17-bin/$noise"
    max_epoch=15
...
#     CUDA_VISIBLE_DEVICES=$cuda  fairseq-train wmt17-bin/$noise \
...
#          --max-epoch $max_epoch --no-epoch-checkpoints   --patience 3 #  --skip-invalid-size-inputs-valid-test &
#     ((cuda=cuda+1))
    ...
    max_epoch=2
    name=${loss:0:2}"_"${noise}"_"${model:0:2}"_"$tf_ratio"_"$sym
    BART_PATH="/home/lptang/fairseq/checkpoints/denoising_bart_wmt/$base_noise/cr_"$noise"_ba_1.0_cetf_wmt_"$i"/checkpoint_best.pt"
    MAX_TOKENS=6000
    UPDATE_FREQ=1
    mkdir log/tf/denoising_bart_wmt/$preix
    CUDA_VISIBLE_DEVICES=1,2 fairseq-train wmt17-bin/$noise \
        ...
        --save-dir checkpoints/denoising_bart_wmt/$preix/$name \
        --tf-ratio $tf_ratio --tensorboard-logdir log/tf/denoising_bart_wmt/$preix/$name \
        --max-epoch $max_epoch --no-epoch-checkpoints  --ngram 1,2 --validate-interval-updates 1000 #--patience 4
    ((cuda=cuda+1))
